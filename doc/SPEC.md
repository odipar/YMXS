...
recorded as an entry of section 7 in place of the chip. A check reports the
errors of 1.11 in the order of 1.12 and the warnings of 6.5 in the order of
6.3. A writer produces a structure free of the conditions of 1.11 that
satisfies the rules of section 6. A reader reads a file of a form into the
structure, and an emitter emits one from it. The host selects a tune (1.2),
calls the player once a frame, and owns bits 7 and 6 of R7 and every timer
outside the tune's (1.10).
...
**1.1** The block is the structure; every declaration is normative, and its
names are the names of this document. Read as Java: a `record` is a tuple of
named fields in declared order; a `sealed interface` with `permits` is a
choice among the records listed; an `enum` is a choice among its constants,
numbered from 0 in declared order; `List<T>` is a sequence numbered from 0;
`Map<K, V>` is a set of pairs, one at most under each key; `OptionalInt` is
an integer, present or absent; `int` and `Integer` are 32-bit signed
integers; `boolean` is true or false; `String` is text, the empty text
included.
...
which the host calls the player, 1 to 2,147,483,647; below 1 is an error of
the structure (1.11).
...
**1.11 Errors of the structure.** A structure with a condition below is
outside what a player plays, and a check reports each condition present as
its line, in the order of 1.12. H is a rate in frames a second, R a row
count, X a repeat row, V a value, M the most of a register (2.1), C a count,
T a rate in ticks a second, W the values a row of a source, U the values a
target reads, N a row number.

| condition | reported as |
|---|---|
| the multi is empty | `a multi of no tunes: a host plays one` |
| the rate of a tune is below 1 | `a rate of H: a player is called at least once a second` |
| the table of a tune is empty | `the tune has no rows: a clock reads one` |
...
| the table of the source of a `Start` is empty | `the source has no rows: a clock reads one` |
...
**2.2 A period over two registers.** A voice's tone period is twelve bits,
the low byte in R0, R2 or R4 and the high four bits in R1, R3 or R5; the
envelope period is sixteen, the low byte in R11 and the high in R12. A row
sets either register of a pair alone (1.6).
...
selects it in the control register of a timer of the MC68901; code 0 stops
the timer.
...
**3.3.6** The mapping of the steps of 4.3 to the registers of the MC68901,
the order of the count and the prescaler within one step, and how a player
claims a timer from the host are left to a later version (8.5).
...
**3.4.2 The place.** Each timer has a place: the number of the row of its
source that the next tick reads. A tick moves it (5.2) and a reset of it
(3.4.3), those two alone. Before the first `Start` or `Retune` with
`placeReset` set on a timer, its place is left to a later version (8.4).

**3.4.3 `placeReset`.** Set: the player sets the place to 0 (4.3), and the
next tick reads the first row of the source. Clear: the place keeps its row
number through a `Retune` and a `Start`, and the next tick reads the row of
that number in the source then connected; two ticks of a two-row source are
one period apart across such a `Start`. A `Stop` leaves the place unchanged.
Where the place is outside the rows of the source connected, what the next
tick reads is left to a later version (8.4); a `Start` under rule 3 leaves
the place at a row of the source it starts.
...
(1.10) from the host, those alone; stop each, so that each is idle; set the
current row to row 0; leave every register at the value the chip has, until
a row sets it.
...
**4.3 The three operations.** An operation is performed on a timer of the
tune (1.10) alone: an operation on another timer leaves that timer and its
registers as they are. A `Retune` on such a timer breaks rule 5, a `Stop`
rule 6, except a `Stop` at the repeat row (rule 6(a)).

A `Stop`: stop the timer. It is idle, its source disconnected, and it raises
...
performs at its ticks is left to a later version (5.4).
...
read runs on: the wrap leaves every timer and every place as they are. Rule
1(d) and rule 6(a) define what the repeat row performs on it.

**4.6 The end.** A tune that plays once ends with its last row: the frame
that reads it reports 0, and every frame after leaves the row, the timers
and the registers as they are and reports -1; R rows are read in frames 0 to
R - 1, and frame R on reports -1. Each timer keeps the condition the last
row left it in; what a host does with a timer running after the end is left
to a later version (8.7). A tune of R rows repeating to RR reads R rows in
its first pass and R - RR in every loop.

---

## 5. What a tick does

**5.1** A tick is one interrupt of a timer, raised once a period at the rate
of its prescaler and count (3.3.1). The player performs this section at a
tick alone.
...
4. Where the source has ended (1.5), stop the timer: the source has run out,
   the timer is idle, the place stays at the last row's number, and the
   register keeps the last value until a row of the tune sets it. A later
   `Start` on the timer with `placeReset` clear, under rule 3, connects a
   source of that row count, and the next tick reads its last row.

**5.3** A tick reads its source alone; the tune's table is read at a frame
(4). A tick leaves the registers of the timer as they are: the timer counts
the next period from the count it has, and a count written by a `Retune`
applies as 3.3.5 defines.

**5.4** What the player performs at a tick of a timer with its source
disconnected (4.3) is left to a later version (8.6). Under rule 5 every tick
...
**6.1** A player assumes the six rules and writes each value as the row or
the tick has it, so a tune that breaks one plays. A breach is a warning
(6.5); a breach of rule 4 leaves every write as it is (4(a)). A check reads
rules 1, 2, 3, 5 and 6 (6.3) of a structure free of the conditions of 1.11;
a structure with one is outside the check.

**6.2 Terms of the rules.**

- (a) An effect runs from step 2 of its `Start` (4.3) until the first of: a
      `Stop` on that timer; a later `Start` on that timer, which replaces
      it; for a source that plays once, the tick that reads its last row
      (5.2 step 4), after which the source has run out.
- (b) An effect runs on the register its target writes.
- (c) A timer is idle from the end of its effect (a) to its next `Start`.
- (d) An effect started on a row runs at step 4 of that row's frame (4.2);
      an effect stopped on a row has ended by step 4 of that frame.
...
- 1(a) A row breaks the rule where it sets a register on which an effect
  ...
  d); the stop row is within the rule, since the effect has ended by step 4,
  and its write sets the register to the value the rows define after the
  effect.
- 1(b) Where the stop row of one effect on a register is the start row of
  another on it, the row leaves the register alone: it belongs to the second
  effect from that row on.
- 1(c) R13 is excluded: a row may set R13 while an effect runs on it, and
  each write restarts the envelope beside the ticks' restarts.
- 1(d) An effect running when the last row is read runs on through the
  ...
  repeat row performs an operation on such a timer: a `Stop` (rule 6(a)), a
  `Start`, or a `Retune` (rule 5).

**Rule 2: where two timers write one register, the writer fixes the
order.**

- 2(a) Two effects run on one register where a `Start` on one timer has a
  target writing the register an effect of another timer runs on. Each tick
  writes the register at its rate; a player writes each tick's value at that
  tick, in the order the ticks arrive.
- 2(b) The writer settles what the two write and in what order. A check
  reports the row where the second starts (6.5); the order is the writer's.

**Rule 3: a `Start` sets `placeReset`, with one exception.**

- 3(a) A `Start` may clear `placeReset` where a `Start` has been performed
  on the timer before, the last had the same target, and the source of the
  last has the row count of the source this one starts; the place then keeps
  its number (3.4.3).
- 3(b) The last `Start` is the last in row order on the timer, its effect
  running or stopped; a `Stop` between leaves the place unchanged (3.4.3),
  so the exception applies across a `Stop`.
- 3(c) A `Start` with `placeReset` clear outside the exception leaves the
  place at a number a row has yet to set (3.4.2), or one outside the rows of
  the source, and what the next tick reads is left to a later version
  (3.4.3).

**Rule 4: a `Start` on a stopped timer sets `timerReset`.**

- 4(a) A timer is stopped where idle (6.2 c). Where the rows leave open
...
- 5(a) A `Retune` on an idle timer breaks the rule: a timer before its first
  `Start`, after a `Stop`, or after its source has run out (5.2 step 4). It
  starts the timer with its source disconnected (4.3).

**Rule 6: a `Stop` on a timer is the first `Stop` after a `Start` on it,
with one exception.**

- 6(a) The repeat row may perform a `Stop` on any timer, one the tune has
  yet to start included.
- 6(b) A `Stop` of a source that plays once and has run out (5.2 step 4) is
  within the rule.
...
Initial condition: the model of every timer is empty, its effect and its
last `Start` absent. For each row N from 0 to the last:
...
3. For each timer with an effect in the model, A to D: close its run; the
   effect stays for step 4.
4. Where the tune repeats: for each timer with an effect in the model
   ...
   repeat row leaves the timer alone.

The report is the lines in the order reported.

**6.4 The reckoning.** A source that plays once has a start row alone; a
check reckons its end row from the rate. For a `Start` at row S of a source
of R rows, divisor D, count C, tune rate H: U = S + F, F = (R × D ×
counted(C) × H + 153,600 + 2,457,599) divided by 2,457,600, the arithmetic
exact. The reckoning reads the prescaler and count of the `Start`; a
`Retune` leaves U unchanged. Four rows at a divisor of 4, a count of 100 and
50 Hz yield F = 1. A warning that rests on the reckoning ends with the
suffix `, which rests on how long a source that plays once runs, reckoned
from its rate`.
...
|---|---|---|
| 1 | one row of the run, row F, sets the register while the effect runs (6.3), alone among the rows of the run | `row F: Timer X runs on Rn, and this row sets it`, with the suffix where the source plays once |
...
| 1(d) | the tune repeats to row T, the effect of Timer X runs when the last row has been read, and row T leaves Timer X alone | `the tune repeats to row T, and Timer X runs on Rn when its last row has played: the wrap resumes with the timer running from the pass before`, with the suffix where the source plays once |
...
| 3 | a `Start` at row N with `placeReset` clear on a timer before any `Start` on it | `row N: Timer X starts a source without the place's reset, and this timer has run none: the place is where the player left it` |
...
| 5 | a `Retune` at row N on an idle timer | `row N: Timer X retunes an effect that is idle: a rate written to a timer with no source on it starts that timer with no source to run`, with the suffix where the effect in the model for the timer, one a `Stop` has yet to remove, has a source that plays once and N is at or above U (6.3), run out by the reckoning |
| 6 | a `Stop` at row N on a timer whose effect is absent from the model, N other than the repeat row | `row N: Timer X stops an effect this timer has not started` |

A `Start` yields at most one rule 3 line, the first condition met in table
order. Rules 1, 2 and 5 are read with the effect running until U (6.3); rule
6 with the effect in the model until a `Stop` or `Start` removes it, so a
`Stop` of a source run out by the reckoning is within rule 6 (6(b)). The
rows of one run are one line, reported where the run closes, so after lines
of later rows; the start row of a replacing `Start` counts for the new
effect's run (1(a)). Of a multi, a check reports each tune's lines in number
order, prefixed `tune N: ` where the multi has more than one tune. Note: an
error of a multi of one tune has the prefix (1.12); a warning of it is
unprefixed.
...
The rule 1 lines follow the rule 2 and rule 6 lines since their runs close
after the last row, Timer A before Timer B; the start row 2 of Timer B
counts among its rows; row 1 leaves both timers alone, so both rule 1(d)
lines are reported.
...
**7.1** A recorder performs the frames of section 4 of one tune with every
register write and every timer operation recorded in place of the chip, and
produces the record: the figures of the tune once, then one entry a frame in
order. The ticks and their writes are outside the record. Of a multi, it
performs the tune the host selects (1.2).

**7.2 The record.** Lines of JSON, one entry a line: spaces inside strings
alone, integers in decimal, the keys of each object in the order this
section lists them, `true`, `false` and `null` as JSON defines them, a line
feed, byte 10, ending every line. The first line is the figures of the tune
...
frames produce one record, line for line.
...
the rows of the source connected (3.4.2, 3.4.3).
...
**8.6** What a player performs at a tick of a timer with its source
disconnected (5.4), and whether a tick occurs between two steps of an
operation or between an operation and the register writes of a frame (4.3).
